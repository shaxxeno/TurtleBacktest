...
def getStratStats(log_returns: pd.Series, risk_free_rate: float = 0.02):
    # Total Returns, Mean Annual Returns, Annual Volatility
    stats = {'tot_returns': np.exp(log_returns.sum()) - 1, 'annual_returns': np.exp(log_returns.mean() * 252) - 1,
...
class TurtleSystem:
    ...
    def __init__(self, tickers, init_account_size=10000, risk_level=2, r_max=0.02,
                 ...
                 start='2000-01-01', end='2022-12-31', shorts=True):
    ...
    def _get_data(self):
        # Get data for all tickers from YFinance
            ...
            if position['long']:
                ...
                # Check to pyramid existing position
                    ...
                    if price >= position['entry_price'] + position['entry_N']:
                        ...
                        stop_price = price - self.risk_level * N
                        avg_price = (position['entry_price'] * position['shares'] + shares * price) / (
...
def sp500_tickers():
    tickers = ['T', 'WMT', 'ECL', 'BIIB', 'WFC', 'NKE', 'CAT', 'NVDA', 'KO', 'GOOG']
    return tickers


def crypto_tickers():
    tickers = ['BTC-USD', 'ETH-USD', 'DOT-USD', 'ATOM-USD', 'ADA-USD']
    return tickers


sys = TurtleSystem(sp500_tickers(), init_account_size=10000, start='2000-01-01')
sys.run()
port_values = sys.get_portfolio_values()
returns = port_values / port_values.shift(1)
log_returns = np.log(returns)
cum_rets = log_returns.cumsum()
example = yf.Ticker('SPY').history(start=sys.start, end=sys.end)
trades = sys.get_transactions()
grouped_data = trades.groupby(['Ticker'])['Returns'].agg({'count', 'mean', 'median', 'std', 'skew'})
grouped_data.to_csv('trades.csv')


def frst_graph():
    # Compare to SPY baseline
    example['returns'] = example['Close'] / example['Close'].shift(1)
    example['log_returns'] = np.log(example['returns'])
    example['cum_rets'] = example['log_returns'].cumsum()

    plt.figure(figsize=(12, 8))
    plt.plot((np.exp(cum_rets) - 1) * 100, label='Turtle Strategy')
    plt.plot((np.exp(example['cum_rets']) - 1) * 100, label='SPY')
    plt.xlabel('Date')
    plt.ylabel('Returns (%)')
    ...
    spy_stats = getStratStats(example['log_returns'])
    df_stats = pd.DataFrame(stats, index=['Turtle'])
    df_stats = pd.concat([df_stats, pd.DataFrame(spy_stats, index=['BTC-USD'])])
    df_stats.to_csv('df_stats.csv')


def scnd_graph():
    mean = trades['Returns'].mean() * 100
    median = np.median(trades['Returns']) * 100

    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    plt.figure(figsize=(12, 8))
    plt.hist(trades['Returns'] * 100, bins=50)
    plt.axvline(mean, c=colors[1], label=f'Mean={mean:.1f}%')
    plt.axvline(median, c=colors[2], label=f'Median={median:.1f}%')
    plt.ylabel('Frequency')
    plt.xlabel('Returns (%)')
    plt.title(f'Histogram of Trade Results N={len(trades)}')
    plt.legend()
...
def thrd_graph():
    ...
    for i, c in enumerate(category):
        vals = trades[c].unique()
        for v in vals:
            data = trades.loc[trades[c] == v]
